...
void DRV_OC0_Initialize(void)
{
    ...
    PLIB_OC_PulseWidth16BitSet(OC_ID_3, 516);

    /* Setup Interrupt */
    PLIB_INT_SourceEnable(INT_ID_0, INT_SOURCE_OUTPUT_COMPARE_3);
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_OC3, INT_PRIORITY_LEVEL1);
    PLIB_INT_VectorSubPrioritySet(INT_ID_0, INT_VECTOR_OC3, INT_SUBPRIORITY_LEVEL0);
}
...
{
    /* Setup OC0 Instance */
    PLIB_OC_ModeSelect(OC_ID_4, OC_DUAL_COMPARE_SINGLE_PULSE_MODE);
    ...
    PLIB_OC_PulseWidth16BitSet(OC_ID_4, 31250);

    /* Setup Interrupt */
    PLIB_INT_SourceEnable(INT_ID_0, INT_SOURCE_OUTPUT_COMPARE_4);
    PLIB_INT_VectorPrioritySet(INT_ID_0, INT_VECTOR_OC4, INT_PRIORITY_LEVEL4);
...
void DRV_OC1_CompareValuesDualSet(uint32_t priVal, uint32_t secVal)
{
    ...
    PLIB_OC_Buffer16BitSet(OC_ID_4, (uint16_t)priVal);

    /* Updating secondary compare value for 16 bit mode */	
    PLIB_OC_PulseWidth16BitSet(OC_ID_4, (uint16_t)secVal);
}
